...
const navItems = [
  ...
  { href: '/admin/pedidos', icon: ClipboardList, label: 'Pedidos', exact: false },
  { href: '/admin/productos', icon: ShoppingBag, label: 'Productos', exact: false },
  { href: '/admin/configuracion', icon: Settings, label: 'Configuracion', exact: false },
];
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-gray-900 text-white fixed inset-y-0 left-0">
        <div className="p-6 border-b border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white" style={{ fontWeight: 700 }}>Takillero</p>
              <p className="text-gray-400" style={{ fontSize: '12px' }}>Admin Panel</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-800 space-y-3">
          ...
          >
            <span className={`w-2.5 h-2.5 rounded-full ${settings.isOpen ? 'bg-green-400' : 'bg-red-400'}`} />
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-gray-900 text-white px-4 py-3 flex items-center justify-between">
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full ${
            settings.isOpen ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
          }`}
        ...
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 bg-gray-900 text-white flex flex-col">
            ...
            <div className="p-4 border-t border-gray-800 space-y-2">
              <Link href="/" onClick={() => setSidebarOpen(false)} className="block text-gray-400 px-4 py-2" style={{ fontSize: '13px' }}>
                Ver tienda
              </Link>
              <button onClick={handleLogout} className="text-gray-400 px-4 py-2" style={{ fontSize: '13px' }}>
                Cerrar sesion
              </button>
            </div>
          </div>
      ...
      {/* Main content */}
      <main className="flex-1 lg:ml-64 pt-14 lg:pt-0 min-h-screen overflow-auto">
